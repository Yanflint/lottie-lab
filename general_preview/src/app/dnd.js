...
async function processFilesSequential(refs, files) {
  ...
  for (const f of files) {
    ...
    const isJson = f.type === 'application/json' || f.name?.endsWith?.('.json') || f.type === 'text/plain' || f.name?.endsWith?.('.lottie');
  ...
  if (imgFile) {
    ...
    try { const { afterTwoFrames } = await import('./utils.js'); await afterTwoFrames(); } catch {}
  }
  // Load each Lottie sequentially
