...
async function processFilesSequential(refs, files) {
  // Split into background image (first image) and all JSON files
  let imgFile = null;
  const jsonFiles = [];
  ...
  // 2) Read all lottie JSON files and add them to the list
  if (jsonFiles.length) {
    const items = [];
    for (const jf of jsonFiles) {
      ...
      } catch(e) { console.warn('Bad JSON file', jf?.name, e); }
    }
    if (items.length) {
...
export function initDnd({ refs }) {
  ...
  document.addEventListener('paste', async (e) => {
    ...
    if (textCandidate) { try { const json = JSON.parse(textCandidate); const { addLottieItems } = await import('./multi.js'); addLottieItems({ refs }, [{ name: 'pasted.json', data: json }]); setPlaceholderVisible(refs, false); try { const { afterTwoFrames } = await import('./utils.js'); await afterTwoFrames(); document.dispatchEvent(new CustomEvent('lp:content-painted')); } catch {} } catch {} } = await import('./utils.js'); await afterTwoFrames(); await afterTwoFrames(); document.dispatchEvent(new CustomEvent('lp:content-painted')); } catch {} } catch {} }
  });
}
