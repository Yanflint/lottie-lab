...
async function processFilesSequential(refs, files) {
  ...
  if (imgFile) {
    ...
    try { const { afterTwoFrames } = await import('./utils.js'); await afterTwoFrames(); document.dispatchEvent(new CustomEvent('lp:bg-updated')); } catch {}
  }
...
export function initDnd({ refs }) {
  ...
  document.addEventListener('paste', async (e) => {
    ...
    if (textCandidate) { try { const json = JSON.parse(textCandidate); setLastLottie(json); await addLayerFromData(refs, json); setPlaceholderVisible(refs, false);
