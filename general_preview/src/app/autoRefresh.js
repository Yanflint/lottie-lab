...
async function fetchRev(){
  for (const base of API_CANDIDATES){
    try{
      const u = `\${base}?id=last&rev=1&_=\${Date.now()}`;
...
async function fetchStableLastPayload(maxMs=2000){
  ...
  while (Date.now() < deadline){
    ...
    const pr = await fetch(`\${API_CANDIDATES[0]}?id=last&_=\${Date.now()}`, { cache: 'no-store' });
    ...
    let revNow = '';
    try{
      const rr = await fetch(`\${API_CANDIDATES[0]}?id=last&rev=1&_=\${Date.now()}`, { cache: 'no-store' });
  ...
  const pr2 = await fetch(`\${API_CANDIDATES[0]}?id=last&_=\${Date.now()}`, { cache: 'no-store' });
