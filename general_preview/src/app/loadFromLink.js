...
async function fetchStableLastPayload(maxMs=2000){
  ...
  while (Date.now() < deadline){
    const pr = await fetch(`https://functions.yandexcloud.net/d4eafmlpa576cpu1o92p?id=last&_=${Date.now()}`, { cache: 'no-store' });
    ...
    let revNow = '';
    try{
      const rr = await fetch(`https://functions.yandexcloud.net/d4eafmlpa576cpu1o92p?id=last&rev=1&_=${Date.now()}`, { cache: 'no-store' });
  ...
  // final attempt
  const pr2 = await fetch(`https://functions.yandexcloud.net/d4eafmlpa576cpu1o92p?id=last&_=${Date.now()}`, { cache: 'no-store' });
...
export async function initLoadFromLink({ refs, isStandalone }) {
  ...
  if (id) {
    ...
    if (id === 'last' || id === '__last__') {
      ...
      const r = await fetch(`https://functions.yandexcloud.net/d4eafmlpa576cpu1o92p?id=${encodeURIComponent(id)}&_=${Date.now()}`, { cache: 'no-store' });
